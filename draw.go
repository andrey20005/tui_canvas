...
func (c *Canvas) DrawLine(x0, y0, x1, y1 int, color RGB) {
	dx := abs(x1 - x0)
	dy := abs(y1 - y0)
...
func (c *Canvas) DrawRect(x, y, w, h int, color RGB) {
	if w <= 0 || h <= 0 {
		return
	}
	// Рисуем 4 стороны
...
func (c *Canvas) DrawCircle(cx, cy, r int, color RGB) {
	if r < 0 {
		return
	}
...
func (c *Canvas) drawCirclePoints(cx, cy, x, y int, color RGB) {
...
func (c *Canvas) drawHLine(x1, x2, y int, color RGB) {
...
func (c *Canvas) DrawEllipse(x, y, w, h int, color RGB) {
	if w <= 0 || h <= 0 {
		return
	}
...
// w, h — ширина и высота прямоугольника.
// Эллипс касается всех четырёх сторон прямоугольника, но не выходит за его пределы.
func (c *Canvas) FillEllipse(x, y, w, h int, color RGB) {
	if w <= 0 || h <= 0 {
		return
	}
...
type FigureShaderFn func(absX, absY float64, relX, relY float64, idxX, idxY uint) (RGB, float64)
